...
// August 29, 2024 - Updated CORS to allow only specific origins
...
//   if (!shop) {
//     return json({ ok: false, error: "shop missing" }, { status: 400, headers: cors(request) });
//   }
...
//     body = await request.json();
//   } catch {
//     return json({ ok: false, error: "Invalid JSON" }, { status: 400, headers: cors(request) });
//   }
...
//   if (!shop) return json({ ok: false, error: "shop is required" }, { status: 400, headers: cors(request) });
//   if (!Number.isInteger(buyQty) || buyQty < 1) return json({ ok: false, error: "offer.buyQty must be >= 1" }, { status: 400, headers: cors(request) });
//   if (!Number.isInteger(getQty) || getQty < 1) return json({ ok: false, error: "offer.getQty must be >= 1" }, { status: 400, headers: cors(request) });
//   if (!remainingMany || !remainingOne || !unlocked) {
//     return json({ ok: false, error: "messages.remainingMany/remainingOne/unlocked are required" }, { status: 400, headers: cors(request) });
//   }
...
export const action = async ({ request }) => {
  ...
  if (!Number.isInteger(buyQty) || buyQty < 1) {
    return json(
      { ok: false, error: "offer.buyQty must be >= 1" },
      { status: 400, headers: cors(request) }
    );
  }
  if (!Number.isInteger(getQty) || getQty < 1) {
    return json(
      { ok: false, error: "offer.getQty must be >= 1" },
      { status: 400, headers: cors(request) }
    );
  }
  if (!remainingMany || !remainingOne || !unlocked) {
    return json(
      { ok: false, error: "messages.remainingMany/remainingOne/unlocked are required" },
      { status: 400, headers: cors(request) }
    );
  }
